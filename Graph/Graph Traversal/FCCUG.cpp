...
//Recources
int vertex_number;
vector< int > VertexList(5000, 0);
void dfs_AL_fccug(int u, int& color, set<int>& dfs_tree_vertexes){ //MOST EFFICIENT
	VertexList[u] = color;  //Mark visited vertex
	dfs_tree_vertexes.insert(u);
	for(auto& i : AdjacencyList[u]){ //Iterate through all connected vertexes 
		if( !( VertexList[i] )) {
			dfs_AL_fccug(i, color, dfs_tree_vertexes);
		} //If vertex #i was not visited yet, go to i
	}
}
//Find connected components in undirected graph
vector< set <int> > fccug(){
	vector< set <int> > components;
	int number_of_components = 0;
	VertexList.assign(VertexList.size(), 0);
	for(int i = 0; i < vertex_number; i++){
		if(VertexList[i] == 0){
			set<int> component;
			dfs_AL_fccug(i, number_of_components + 1, component);
